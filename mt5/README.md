...
`slPoints` / `tpPoints` default to 200 / 100. On a 2-decimal gold quote (`SYMBOL_POINT` 0.01) that is $2.00 stop loss and $1.00 take profit. Those distances clear a ~$0.50 live book with equal mark travel after buy-at-ask / sell-at-bid fills. The EAs also raise the distance to `SYMBOL_TRADE_STOPS_LEVEL` so the broker does not reject the order.
...
| `InpSLPoints` | `200` | $2.00 on a 0.01 point gold quote. 0 means "use the signal" |
| `InpTPPoints` | `100` | $1.00 on a 0.01 point gold quote. 0 means "use the signal" |
...
SL/TP on the new order: EA input if > 0, else signal points, else 200 / 100, then clamp to `SYMBOL_TRADE_STOPS_LEVEL`. Confirm the attached SL and TP on the ticket in the Trade tab. If they are missing, the broker rejected stops and the scalp may not close. If a chart was already attached with the old 40 / 60, 60 / 100, 80 / 120, or 600 / 800 inputs, reset those inputs or the EA will keep the old stops.
...
| `InpSLPoints` / `InpTPPoints` | `200` / `100` | same resolve rule as the leader |
...
| `GOLD_SL_POINTS` | `200` | rides on `/signal` |
| `GOLD_TP_POINTS` | `100` | rides on `/signal` |
