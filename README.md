...
This gold process is a small in-out scalp, not a swing hold. Jev is asked whether the small take profit is more likely to print before the stop, and answers about once a second (`GOLD_INTERVAL_MS`). The open ticket stays on until the $1.00 stop or the $0.60 take profit. The latest buy or sell is the side of the next ticket.

`GET /signal` always includes `slPoints` and `tpPoints` (defaults 100 and 60, or `GOLD_SL_POINTS` / `GOLD_TP_POINTS`). On a 2-decimal gold quote (`SYMBOL_POINT` 0.01) that is $1.00 stop loss and $0.60 take profit. The take profit is the near exit so a small push banks. The stop is wider because a live bounce of about $0.80 was tagging a $0.40 stop and knocking out the scalp before that push. The MT5 EAs attach both on every new gold market order.
