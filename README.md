...
`GET /signal` always includes `slPoints` and `tpPoints` (defaults 200 and 100, or `GOLD_SL_POINTS` / `GOLD_TP_POINTS`). On a 2-decimal gold quote (`SYMBOL_POINT` 0.01) that is $2.00 stop loss and $1.00 take profit. Those distances clear a ~$0.50 live book with equal mark travel after paying the spread. Set `GOLD_FILL_MODE=mid` only for mid-fill comparison; it is not the default proof path. The MT5 EAs attach both on every new gold market order.
