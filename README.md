...
This gold process is a small in-out scalp, not a swing hold. Jev is asked whether the small take profit is more likely to print before the small stop, and answers about once a second (`GOLD_INTERVAL_MS`). The open ticket stays on until the $0.40 stop or the $0.60 take profit. The latest buy or sell is the side of the next ticket.

`GET /signal` always includes `slPoints` and `tpPoints` (defaults 40 and 60, or `GOLD_SL_POINTS` / `GOLD_TP_POINTS`). On a 2-decimal gold quote (`SYMBOL_POINT` 0.01) that is $0.40 stop loss and $0.60 take profit, still above a typical ~15 pip / $0.15 spread. A quiet live spot can move less than $1 for several minutes, so the wider $0.80 / $1.20 exits were sitting through a finished scalp. The MT5 EAs attach both on every new gold market order so each scalp can take a small win or cut, instead of sitting for a multi dollar move.
