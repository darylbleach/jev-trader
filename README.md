...
This gold process is a small in-out scalp, not a swing hold. Jev is asked whether the take profit is more likely to print before the stop, and answers about once a second (`GOLD_INTERVAL_MS`). The open ticket stays on until the $2.50 stop or the $1.50 take profit. The latest buy or sell is the side of the next ticket.

`GET /signal` always includes `slPoints` and `tpPoints` (defaults 250 and 150, or `GOLD_SL_POINTS` / `GOLD_TP_POINTS`). On a 2-decimal gold quote (`SYMBOL_POINT` 0.01) that is $2.50 stop loss and $1.50 take profit. Dummy buys fill at the ask and sells fill at the bid; stops and targets are judged on the exit side (bid for longs, ask for shorts). The take profit clears a typical ~$0.50 Swissquote-style spread with about $1.00 of margin. The stop leaves bounce room after that spread hit. The MT5 EAs attach both on every new gold market order.

Dry-run (default) also simulates JevLeader tickets so the gold page can show what would happen without a broker: market open at ask (buy) or bid (sell), and SL/TP exits when the exit side touches. An opposite Jev call does not scratch the open ticket. Open ticket, recent dummy trades, and running P and L are on `GET /status` and the demo page under MT5 dummy (bid/ask fills, not a live broker). Dummy fills reuse `POST /fill` internally and the existing SSE `fill` events. Set `GOLD_DUMMY_MT5=false` to turn that off, or `GOLD_DRY_RUN=false` when a real EA posts `/fill`. A broker stop or target posted to `/fill` with `reason` `sl` or `tp` flattens the published position so the next decision can open.
