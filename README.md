...
This gold process is a small in-out scalp, not a swing hold. Jev is asked whether the small take profit is more likely to print before the stop, and answers about once a second (`GOLD_INTERVAL_MS`). Dummy proof fills buy at ask and sell at bid (exit on the opposing side). The open ticket stays on until the $2.00 stop or the $1.00 take profit. The latest buy or sell is the side of the next ticket.

`GET /signal` always includes `slPoints` and `tpPoints` (defaults 300 and 200, or `GOLD_SL_POINTS` / `GOLD_TP_POINTS`). On a 2-decimal gold quote (`SYMBOL_POINT` 0.01) that is $3.00 stop loss and $2.00 take profit. Those distances clear a ~$0.50 live book with equal mark travel after paying the spread (cash breakeven WR ≈ 60%). Set `GOLD_FILL_MODE=mid` only for mid-fill comparison; it is not the default proof path. The MT5 EAs attach both on every new gold market order.

Dry-run (default) also simulates JevLeader tickets so the gold page can show what would happen without a broker: market open at mid, and SL/TP exits when price touches. An opposite Jev call does not scratch the open ticket. Open ticket, recent dummy trades, and running P and L are on `GET /status` and the demo page under MT5 dummy (simulated, not a live broker). Dummy fills reuse `POST /fill` internally and the existing SSE `fill` events. Set `GOLD_DUMMY_MT5=false` to turn that off, or `GOLD_DRY_RUN=false` when a real EA posts `/fill`. A broker stop or target posted to `/fill` with `reason` `sl` or `tp` flattens the published position so the next decision can open.
